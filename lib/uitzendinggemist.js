...
var UitzendingGemist = function(id, cb) {
  ...
  this.getSessionURL = function() {
    var requestURL = 'http://pi.omroep.nl/info/stream/aflevering/' + self.episodeIdentifier + '/' + self.hash;
    request({
      url: requestURL
    }, function(err, res, body) {
      if (err || res.statusCode !== 200) {
      ...
      var urls = xml2js.parseString(body, function(err, res) {
        if (err || !res.streams || !res.streams.stream || !res.streams.stream.length) {
          throw new Error('Could not parse XML from Session URLS. URL: ' + requestURL);
        }
